...
if($conn){
    if($email!='' && $password!='' && $username!=''){
        $checkExist="SELECT id from `db201` WHERE email='{$email}'";
        $executeCheckExist=mysqli_query($conn,$checkExist);
        if(!mysqli_num_rows($executeCheckExist)>0){
            regeneratetoken:
            $token= uniqid (rand (),true);
            $checkExist2="SELECT id from `db201` WHERE token='{$token}'";
            $executeCheckExist2=mysqli_query($conn,$checkExist2);
            if(!mysqli_num_rows($executeCheckExist2)>0){
                $hashPassword=password_hash($password, PASSWORD_DEFAULT);
                $insertValue="INSERT INTO `db201`(`token`, `email`, `pswd`, `username`, `date`, `time`) VALUES ('{$token}','{$email}','{$hashPassword}','{$username}','".date('Y/m/d')."','".date('H:i:s')."')";
