...
if($conn){
    if($email!='' && $password!=''){
        $checkExist="SELECT `token`, `email`,`username`,`pswd` FROM `db201` WHERE email='{$email}' ";
        ...
        if(mysqli_num_rows($executeCheckExist)>0){
            ...
            if(password_verify($password,$row['pswd'])){
                setcookie("advertiseremail", $row['email'],  time() + (86400 * 7), "/");
                setcookie("advertisertoken", $row['token'],  time() + (86400 * 7), "/");
                setcookie("advertiserusername", $row['username'],  time() + (86400 * 7), "/");
